  ...
  if ($('#vocational_training_citys_students_pie').length) {
    // 廠商行業別分布情形
    (function vocational_training_citys_students_pie() {


      var dom = document.getElementById("vocational_training_citys_students_pie");
      var vocational_training_citys_students_pie = echarts.init(dom, 'wda_data', {
        renderer: 'svg',
        useDirtyRect: true,
        locale: 'EN'
      });
      ...
      option = {
        ...
        title: {
          text: 'XXX 行業別 - 減班休息廠商與核定充電再出發各縣市家數占比',
          textStyle: {
            fontSize: 24,
            fontWeight: 'bolder',
            color: tu_dark,
      ...
      if (option && typeof option === 'object') {
        vocational_training_citys_students_pie.setOption(option);
      }

      window.addEventListener('resize', function() {
        vocational_training_citys_students_pie.resize();
